...
class FaceRecognizer(object):
    ...
    for person_name in person_names:
      ...
      for picture_file in pictures:
        ...
        if len(face_bboxes) == 0:
          ...
          print('{}, face_bboxes: {}'.format(person_name + '/' + picture_file,
                                             face_bboxes))
  ...
  def recognize_person(self, image, distance_threshold=0.6):
    ...
    if self.fitting == 'svm':
      ...
      closest_distances = self.classifier.kneighbors(face_encodings,
                                                     n_neighbors=1)
      are_matches = [
      ...
      return [(pred, loc) if rec else ("unknown", loc)
              for pred, loc, rec in zip(self.classifier.predict(face_encodings),
